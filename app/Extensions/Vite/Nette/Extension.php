...
namespace App\Extensions\Vite\Nette;

use App\Extensions\Vite\AssetFilter;
use App\Extensions\Vite\ManifestFileDoesNotExistsException;
use App\Extensions\Vite\Service;
use App\Extensions\Vite\Tracy\VitePanel;
use Nette;
...
final class Extension extends CompilerExtension
{
	...
	private function prepareBasePath(string $manifestFile): string
	{
		if ($this->config->basePath === null) {
			return str_replace(Nette\Safe::realpath($this->config->wwwDir), '', dirname($manifestFile)) . 'Extension.php/';
		}
